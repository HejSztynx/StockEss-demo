...
from app.globals import FORECAST_PERIODS, SEQUENCE_LENGTHS_FOR_PERIODS, RETRAINING_RATE
import numpy as np
...
from app.globals import raw_data, processed_data, models, scalers
...
def get_predictions_for_date(ticker, target_date):
    ...
    if predictions.get("prediction1y") is None:
        ...
        if shorter_preds:
            predictions["prediction1y"] = round(sum(shorter_preds) / len(shorter_preds), 2)

    return predictions


def get_data_slice(ticker, start, end):
    return raw_data[ticker][(raw_data[ticker].index >= start) & (raw_data[ticker].index <= end)].copy()
